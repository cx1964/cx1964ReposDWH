...
from (
  ...
  select  
         ...
         -- neem vanwege de group by de max alle Meta gegevens van de SATs
         ,max(data.meta_record_source)        as meta_record_source 
         ,max(data.meta_load_date)            as meta_load_date
	   ,max(data.meta_create_time)          as meta_create_time
  from (
