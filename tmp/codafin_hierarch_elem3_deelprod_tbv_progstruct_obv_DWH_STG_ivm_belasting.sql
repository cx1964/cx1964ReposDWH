...
-- ****************************************************************
-- *** Onderstaande queries alleen tbv testen bovenstaande query --
-- ****************************************************************
...
where 1=1
  and (  -- Geldigheidscheck: alleen Geldig='ja'
         case
           when year(getdate()) * 10000 + month(getdate())
             between 
               --(case when Prog.startyear &gt; 0 then Prog.startyear else 1900 end * 10000 + Prog.startperiod)
...
-- query Greisy uit uit SSRS rapport CI40826 "CODA Element 3 programmastructuur 2020-2024"
-- Declare @Geldig as NVARCHAR(10) = 'Ja'; -- Ja , Nee of * 
select * 
from 
(select
  Prog.l1name as ProgrammaCode,
...
from
  /*
  dbo.oas_element oas_element,
  dbo.oas_grplist oas_grplist,
  dbo.oas_himlist oas_himlist
  */
  dwh_stg.codafin12.oas_element oas_element,
  dwh_stg.codafin12.oas_grplist oas_grplist,
  dwh_stg.codafin12.oas_himlist oas_himlist 
where
  ...
  oas_element.cmpcode  = oas_grplist.cmpcode and
  oas_element.cmpcode  = 'PZH' and
  oas_himlist.code     = 'PZHPROG2024'
) as Prog

) R

where  (R.Geldig = '*') or (R.Geldig = 'ja')
       -- mijn test situtaie
       and R.TaakCode = '9-1-1' -- beleidsprestatiecode
order by
  DeelproductCode;
go 
